...
Begin VB.Form frmMakeWave 
   ...
   Begin VB.Frame frameOptions 
      ...
      Begin VB.TextBox txtboxEachZero 
         ...
         TabIndex        =   3
         Text            =   "1000"
         ToolTipText     =   "The time for each zero interval (time in micro seconds)"
         Top             =   885
         Width           =   1095
      End
...
Dim arrNewPointsForAmplitude&(10000)
...
subCalculateNewPointsAmplitude
...
For I = intStart% To intEnd%
    ...
    For B = 1 To intAmountofReps% Step intSteps%
        ...
        '22050hz)
        'longintTotalFileSize& = longintTotalFileSize& + 4 * Int((Pi# * 2) / (arrNewPointsForFreq&(I) * (2 * Pi# / 22050)))
...
For I = intStart% To intEnd%
    ...
    If intSteps% = 0 Then intSteps% = intAmountofReps%
'------------------
'arrNewPointsForAmplitude&(I)
    numNewAmplitude# = (arrNewPointsForAmplitude&(I) / 100) * 30000
'------------------
    'this loop repeats the waves
    For B = 1 To intAmountofReps% Step intSteps%
        ...
        For O# = 0 To (Pi# * 2) Step (arrNewPointsForFreq&(I) * (2 * Pi# / 22050))
            ...
            longintPointOnSineW# = Int(numNewAmplitude# * Sin(O#)) '-----------------
            'if the value is less than or equal to zero
            'then add &HFFFF (modulus system)
            If longintPointOnSineW# <= 0 Then longintPointOnSineW# = 65535 + longintPointOnSineW#
            '---------------------------------------------
            '***Updated Code***
            '---------------------------------------------
            'converts the longintPointOnSineW# number into
            'two characters. I received a comment from
            'someone about making my program 5 times
            'faster. I mailed him back but received no word
            'from him. So I decided to see if I could
            'optimise my code any more. So I came up with
            'this.
            strTemp2$ = Chr$(Int(longintPointOnSineW# / 256))
            strTemp3$ = Chr$(longintPointOnSineW# - Int(longintPointOnSineW# / 256) * 256)
            '---------------------------------------------
            'write the converted values to file
            Print #1, strTemp3$ + strTemp2$ + strTemp3$ + strTemp2$;
        Next O#
    Next B
203
Next I
...
'-------------------------------------------------------
'-------------------------------------------------------
'Basically the same as subCalculateNewPoints, but this
'calculates the amplitude instead
'-------------------------------------------------------
Private Sub subCalculateNewPointsAmplitude()
'set the array up
For I = 0 To 10000
    arrNewPointsForAmplitude&(I) = 100
Next I
'set K=0 (i.e. the first point)
K = 0
subFindNextValueAmplitude (K)
...
For K = intStart% To 10000
    'is there is no point then goto the next K
    If arrPointsForAmplitude&(K) = 100 Then GoTo 10
    'find the next non-zero value
    subFindNextValueAmplitude (K + 1)
    'if there is no next nob-zero value then store the
    'current point and continue (otherwise the very last
    'point will be disguarded)
    If Val(strTemp3$) = -1 Then
        arrNewPointsForAmplitude&(K) = arrPointsForAmplitude&(K)
        intEnd% = K
        GoTo 10
    End If
    'if the values are set to -1 (i.e. user plotted zero
    ...
    'to zero (so that zero values are not counted as null
    'values)
    If arrPointsForAmplitude&(K) = -1 Then arrPointsForAmplitude&(K) = 100
    If arrPointsForAmplitude&(Val(strTemp3$)) = -1 Then arrPointsForAmplitude&(Val(strTemp3$)) = 100
    OldK = K
    'if the two points are consecutive then there are no
    'points between, therefore continue with the loop
    If Val(strTemp3$) - (K + 1) = 0 Then
    ...
    Else
        ...
        'all the points between them are equal to the two
        'points
        If arrPointsForAmplitude&(K) = arrPointsForAmplitude&(Val(strTemp3$)) Then
            For G = K To Val(strTemp3$)
                arrNewPointsForAmplitude&(G) = arrPointsForAmplitude&(K)
            Next G
            K = Val(strTemp3$) - 1
        Else
            'if the current point is less than the next
            ...
            If arrPointsForAmplitude&(K) < arrPointsForAmplitude&(Val(strTemp3$)) Then
                For G = K To Val(strTemp3$)
                    arrNewPointsForAmplitude&(G) = arrPointsForAmplitude&(K) + (arrPointsForAmplitude&(Val(strTemp3$)) - arrPointsForAmplitude&(K)) * ((G - K) / (Val(strTemp3$) - K))
                Next G
                K = Val(strTemp3$) - 1
            Else
                'if the current point is more than the next
                ...
                'progress linearly downward to the next point
                For G = K To Val(strTemp3$)
                    arrNewPointsForAmplitude&(G) = arrPointsForAmplitude&(K) - (arrPointsForAmplitude&(K) - arrPointsForAmplitude&(Val(strTemp3$))) * ((G - K) / (Val(strTemp3$) - K))
                Next G
                K = Val(strTemp3$) - 1
            End If
        End If
    ...
    'if the points used were set to zero, then set them
    'back to -1
    If arrPointsForAmplitude&(Val(strTemp3$)) = 100 Then arrPointsForAmplitude&(Val(strTemp3$)) = -1
    If arrPointsForAmplitude&(OldK) = 100 Then arrPointsForAmplitude&(OldK) = -1
    'set the end of the project (it will be equal to the
    'last value)
    intEnd% = K
10
...
End Sub
Private Sub subFindNextValueAmplitude(intStartFrom As Integer)
'set a temp string to '-1'
strTemp3$ = "-1"
'find the next point starting from intStart
For J = intStartFrom To 10000
